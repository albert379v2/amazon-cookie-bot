...
try:
    ...
    wait_for_otp = None
    print("⚠️ gmail_reader not found - OTP will need manual entry")
...
if not TOKEN:
    ...
    print("   Local: export BOT_TOKEN='your_token'")
    exit(1)
...
async def handle_captcha_challenge(page):
    ...
    await page.screenshot(path="full_page_captcha.png", full_page=True)
    send_photo_to_bot("full_page_captcha.png", caption="📸 Página completa - contexto del captcha")

    path = await capture_captcha(page)
    if path:
        send_photo_to_bot(path, caption="🎯 Responde con los números de los tiles\nEjemplo: 1 3 5\n\n📸 Primero envío la página completa arriba para que veas el contexto")
...
def get_proxy_config():
    """Get proxy config from environment or return None."""
    proxy_json = os.getenv('PROXY_CONFIG', '')
    if proxy_json:
        try:
...
async def wait_for_otp_email(email):
    ...
    if wait_for_otp is None:
        send_log("❌ Gmail reader not available")
        return None
...
if __name__ == "__main__":
    ...
    # Run Telegram bot in a separate thread so /crear doesn't block
    def run_bot():
